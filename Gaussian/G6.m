clf; close all; 
global delta alpha_X beta_X Ein
%In general, you can generate N random numbers in the 
%interval [a,b] with the formula r = a + (b-a).*rand(N,1).%
rng(0,'twister');
N = 1000;
%choose a [-5 5]
a_random = 0 + (5).*rand(N,1);
%choose b [-5 5]
b_random = 0 + (1).*rand(N,1);
%choose c [-3 3]
c_random =  0 + (1).*rand(N,1);
%choose d [-3 3]
d_random =  0 + (1).*rand(N,1);
%choose sigma [0.1 3]
sigma_random = 0.1 + (3-0.1).*rand(N,1);
%choose sigma [0.1 3]
xi_random = 0 + (10).*rand(N,1);
...
xi0 = x';
fitting_func =  inline(...
        '[(p(1)).*exp(-0.5*(((x - p(6)).^2)/p(5).^2)).*exp(-1i*(p(4).*(x-p(6)).^2 + p(3).*(x-p(6)) + p(2)))]'...
        ,'p','x');
param0 = [sqrt(3), 0, 0, 0,1,0];
paramSeed=param0; %lsqcurvefit(fitting_func,param0,xi0, real(uPDE),[],[]);
uSeed = (paramSeed(1)).*exp(-0.5*(((xi0 - paramSeed(6)).^2)/paramSeed(5).^2)).*exp(-1i*(paramSeed(4).*(xi0-paramSeed(6)).^2 + paramSeed(3).*(xi0-paramSeed(6)) + paramSeed(2)));
a_random(1) = real(paramSeed(1));
b_random(1) = real(paramSeed(2));
c_random(1) = real(paramSeed(3));
d_random(1) = real(paramSeed(4));
sigma_random(1) = real(paramSeed(5));
xi_random(1) = real(paramSeed(6));
plot( xi0, uSeed.*conj(uSeed))

maxit=100;                          %nsoli params
maxitl=100;
etamax=0.9;
lmeth=1; %3;
restart_limit=50;
...
for i = 4:10;
    i
    x0 = [a_random(i); b_random(i); c_random(i); d_random(i); sigma_random(i); xi_random(i)];
    [solp,it_hist,ierr,u_hist]=nsoli(x0,@(solp_)LLE_steady_state_G6(solp_),1e-15*[1,1],sol_parms);
    ...
    %[solp, resnorm, residual, exitflag] = lsqnonlin(@LLE_steady_state_XC6,x0,[],[],options);
    %[solp, fval, exitflag, output] = fsolve(@PGGaussian,x0)
    uVA(:,i) = (solp(1)).*exp(-0.5*(((xi0 - solp(6)).^2)/solp(5).^2)).*exp(1i*(solp(4).*(xi0-solp(6)).^2 + solp(3).*(xi0-solp(6)) + solp(2)));
    ...
    fRHS(:,i) = LLE_steady_state_G6(solp);
...
for i = 1:length(t);
    pVA = xVA(i,:);
    uVA2(:,i) = (pVA(1)).*exp(-0.5*(((xi0 - pVA(6)).^2)/pVA(5).^2)).*exp(1i*(pVA(4).*(xi0-pVA(6)).^2 + pVA(3).*(xi0-pVA(6)) + pVA(2)));
